/* ===== PostAi Chat 2.0 — floating widget met overlay-fix ===== */
(function(){
  ...
  // Veilig naar top-document (escape uit iframes)
  ...
  if(!DOC.getElementById("bms-chat-css")){
    ...
    link.href=(window.BMS_CHAT_CSS_URL||"/chat-widget.css");
    DOC.head.appendChild(link);
  }

  // Helpers
  const el=(t,a={},h="")=>{const n=DOC.createElement(t);Object.entries(a).forEach(([k,v])=>n.setAttribute(k,v));if(h)n.innerHTML=h;return n;};
  const qs=s=>DOC.querySelector(s);
  const escapeHtml=s=>String(s).replace(/[&<>"]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"}[c]));

  // === Overlay (nieuw) ===
  const overlay = el("div",{id:"bms-overlay","aria-hidden":"false"});
  ...
  const launcher = el("button",{id:"bms-chat-launcher",ariaLabel:"Open chat"},
    `<svg viewBox="0 0 24 24" fill="none" width="24" height="24" aria-hidden="true">
      <path d="M12 3c5.5 0 10 3.8 10 8.5S17.5 20 12 20a13 13 0 0 1-3.7-.5L3 21l1.7-4.3A8.6 8.6 0 0 1 2 11.5C2 6.8 6.5 3 12 3Z"
            stroke="currentColor" stroke-width="1.6"/></svg>`);
  const teaser = el("div",{id:"bms-chat-teaser",role:"status"},"Hulp nodig? Sanne helpt je graag.");
  overlay.append(launcher,teaser);
  ...
  // Chatvenster
  const chat = el("div",{id:"bms-chat",role:"dialog","aria-modal":"true","aria-label":"Chat met Sanne"},`
    <div class="hdr">
  ...
  if(!localStorage.getItem("bmsChatWelcomed")){
    ["Hi! Ik ben Sanne. Waar kan ik je mee helpen?","Tip: wil je beste posttijd? Zeg “beste tijd”.","🔒 We bewaren niets zonder jouw actie."]
      .forEach((w,i)=>setTimeout(()=>{append("bot",w);save("bot",w);},i*400));
  ...
  function addFeedback(){
    const fb=el("div",{class:"bms-feedback"},`<span>Was dit nuttig?</span><button aria-label="Ja">👍</button><button aria-label="Nee">👎</button>`);
    fb.querySelectorAll("button").forEach(b=>b.onclick=()=>{fb.innerHTML="Bedankt voor je feedback 💜";});
    body.appendChild(fb);
  }
  function addRestart(){
